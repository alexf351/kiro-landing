...
function buildLocale(L) {
  ...
  // Language menu: this page is the current one. (hreflang/lang use the BCP 47
  // tag, which differs from the locale key for pt-br → pt-BR.)
  out = out.replace(/(<a href="\/" hreflang="en" lang="en" data-lang-link) aria-current="page"/g, '$1')
    .replace(new RegExp(`(<a href="${esc(meta.path)}" hreflang="${esc(meta.lang)}" lang="${esc(meta.lang)}" data-lang-link)`, 'g'), '$1 aria-current="page"');

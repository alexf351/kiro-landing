...
**`.hero-actions`** (below universe): App Store button (`.btn-primary`, with Apple logo + "Download on the App Store"), ghost "See it in action" (→ #showcase), and trust row (★★★★★ "4.8 on the App Store" · "345 lessons across 18 paths"). A `data-herocta="above|below"` attribute on `<html>` flips this block above/below the universe (default `below`).
...
- Stat row (`.proof-top`): **4.8** App Store rating (★★★★★) · count-up "2,075+" hands-on exercises · count-up "18" learning paths. Count-ups animate when scrolled into view (`app.js`, `[data-count]`).
...
- Rating is set to **4.8** (US App Store) per the client; if a global average is shown elsewhere, label it.
